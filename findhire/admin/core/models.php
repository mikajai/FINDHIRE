...
function insertJobPost($pdo, $job_name, $job_description, $posted_by) {
    $sql = "INSERT INTO job_posts (job_name, job_description, posted_by) VALUES (?, ?, ?)";
    $stmt = $pdo->prepare($sql);
    return $stmt->execute([$job_name, $job_description, $posted_by]);
}
...
function updateJobPost($pdo, $job_id, $job_name, $job_description) {
    $sql = "UPDATE job_posts SET job_name = ?, job_description = ? WHERE job_id = ?";
    $stmt = $pdo->prepare($sql);
    return $stmt->execute([$job_name, $job_description, $job_id]);
}
...
function getApplicationsByJobId($pdo, $job_id) {
...
function updateApplicationStatus($pdo, $application_id, $status) {
    $sql = "UPDATE job_application SET status = :status 
